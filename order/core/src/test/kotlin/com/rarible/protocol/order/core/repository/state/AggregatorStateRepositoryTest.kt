...
internal class AggregatorStateRepositoryTest : AbstractIntegrationTest() {
    ...
    @Test
    fun `should save and get reservoir asks state`() = runBlocking<Unit> {
        val noState = repository.getReservoirAsksEventState()
        assertThat(noState).isNull()

        val initState = ReservoirAsksEventFetchState(cursor = "1")
        repository.save(initState)

        var currentState = repository.getReservoirAsksEventState()
        assertThat(currentState).isEqualTo(initState)

        val newState = ReservoirAsksEventFetchState("2")
        repository.save(newState)

        currentState = repository.getReservoirAsksEventState()
        assertThat(currentState).isEqualTo(newState)
    }
}
