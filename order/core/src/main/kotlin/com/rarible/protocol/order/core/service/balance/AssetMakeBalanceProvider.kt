...
class AssetMakeBalanceProvider(
     ...
     */
    private suspend fun handleCryptoPunkMarketOrder(order: Order): EthUInt256? {
        if (
    ...
    private suspend fun handleRaribleOrder(order: Order): EthUInt256? {
        ...
        if (order.make.type is CryptoPunksAssetType) {
            return cryptoPunkAssetMakeStockService.getRaribleCryptoPunkSellMakeStock(
                owner = order.maker,
