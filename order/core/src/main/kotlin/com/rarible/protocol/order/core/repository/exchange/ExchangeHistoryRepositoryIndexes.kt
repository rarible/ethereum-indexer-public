...
object ExchangeHistoryRepositoryIndexes {
    ...
    val ITEM_SELL_DEFINITION: Index = Index()
        .on("${LogEvent::data.name}.${OrderExchangeHistory::make.name}.${Asset::type.name}.${Erc721AssetType::token.name}", Sort.Direction.ASC)
        .on("${LogEvent::data.name}.${OrderExchangeHistory::make.name}.${Asset::type.name}.${Erc721AssetType::tokenId.name}", Sort.Direction.ASC)
        .on("${LogEvent::data.name}.${OrderExchangeHistory::date.name}", Sort.Direction.ASC)
        .on("_id", Sort.Direction.ASC)
        .background()

    val ITEM_BID_DEFINITION: Index = Index()
        .on("${LogEvent::data.name}.${OrderExchangeHistory::take.name}.${Asset::type.name}.${Erc721AssetType::token.name}", Sort.Direction.ASC)
        .on("${LogEvent::data.name}.${OrderExchangeHistory::take.name}.${Asset::type.name}.${Erc721AssetType::tokenId.name}", Sort.Direction.ASC)
        .on("${LogEvent::data.name}.${OrderExchangeHistory::date.name}", Sort.Direction.ASC)
        .on("_id", Sort.Direction.ASC)
        .background()
