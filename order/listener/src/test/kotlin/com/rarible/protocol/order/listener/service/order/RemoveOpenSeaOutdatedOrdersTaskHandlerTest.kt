...
class RemoveOpenSeaOutdatedOrdersTaskHandlerTest : AbstractIntegrationTest() {
    ...
    fun `remove OpenSea orders`() = runBlocking<Unit> {
        ...
        repeat(3) {
            orderRepository.save(createOrder().copy(
        ...
        repeat(5) {
            val orderVersion = createOrderVersion().copy(
                platform = Platform.OPEN_SEA,
                data = createOrderOpenSeaV1DataV1().copy(exchange = exchangeContractAddresses.openSeaV2, nonce = 0),
                type = OrderType.OPEN_SEA_V1,
                createdAt = Instant.EPOCH,
            )
            orderVersionRepository.save(orderVersion).awaitFirst()
        ...
        assertEquals(30, updatedOrdersFromRep.count())
        ...
        updatedOrdersFromRep.forEach {
            val exchange = (it.data as OrderOpenSeaV1DataV1).exchange
            when {
                exchangeContractAddresses.openSeaV1 == exchange ->
                    assertEquals(Instant.ofEpochSecond(1645812000), it.lastUpdateAt)
                exchangeContractAddresses.openSeaV2 == exchange ->
                    assertEquals(Instant.ofEpochSecond(1659366000), it.lastUpdateAt)
                else ->
                    throw IllegalArgumentException("Unexpected OpenSea test exchange")
            }
        }
    }
}