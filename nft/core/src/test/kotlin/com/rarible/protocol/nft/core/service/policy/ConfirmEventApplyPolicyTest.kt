...
internal class ConfirmEventApplyPolicyTest {
    ...
    fun `should say false on event if not any confirms events in list`() {
        ...
        val event = createRandomMintItemEvent().withNewValues(status = EthereumLogStatus.CONFIRMED)
    ...
    fun `should add latest event to list and remove not revertable events`() {
        ...
        val notConfirmEvent1 = createRandomMintItemEvent().withNewValues(status = EthereumLogStatus.PENDING)
        val notConfirmEvent2 = createRandomMintItemEvent().withNewValues(status = EthereumLogStatus.PENDING)
